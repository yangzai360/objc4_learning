...
// 在dyld得到控制之前，内核会映射到主要可执行文件中。
// 我们需要为已经映射到主要可执行文件中的文件制作一个 ImageLoader *
...
//
// dyld的入口点。 内核加载dyld并跳转到__dyld_start，后者设置一些寄存器并调用此函数。
//
//返回__dyld_start跳转到的目标程序中main（）的地址
uintptr_t
